  ...
  return (
    <HoverLift>
      <Link href={`/produits/${product.slug}`} className={cn("group block", className)}>
        {/* "large" gets a bigger radius (matches the hero fix): rounded-2xl
            on an image this size reads as barely-rounded, out of proportion
            with how bold the rest of the redesign's corners are. */}
        <div className="relative">
          <SmartImage
            ...
            wrapperClassName={cn("bg-secondary", size === "large" ? "rounded-[1.5rem]" : "rounded-2xl")}
            sizes={size === "large" ? "(min-width: 1024px) 50vw, 100vw" : "(min-width: 1024px) 25vw, 50vw"}
            className="transition-transform duration-500 group-hover:scale-[1.03]"
          />
          {discountPercent ? (
            <span className="absolute top-3 left-3 rounded-full bg-surface-yellow px-2.5 py-1 text-xs font-semibold text-surface-yellow-foreground">
              -{discountPercent}%
            </span>
          ) : null}
